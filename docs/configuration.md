...
| Event | Actions used |
| --- | --- |
| `pull_request` | `opened`, `reopened`, `labeled`, `unlabeled`, and `synchronize`. `opened` never creates an approval; the `labeled` event GitHub sends for each label present at creation does. |
